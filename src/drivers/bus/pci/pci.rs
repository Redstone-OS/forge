...
/// Procura um dispositivo PCI por Class/Subclass/ProgIF
///
/// # Argumentos
/// * `class_code` - Código de classe (ex: 0x01 para Mass Storage)
/// * `subclass` - Subclasse (ex: 0x06 para SATA)
/// * `prog_if` - Programming Interface (opcional, ex: 0x01 para AHCI)
    ...
    devices
        ...
        .find(|d| {
            d.class_code == class_code
                && d.subclass == subclass
                && prog_if.map_or(true, |pi| d.prog_if == pi)
        })
        .cloned()
}

/// Procura todos os dispositivos PCI por Class/Subclass
pub fn find_all_by_class(class_code: u8, subclass: u8) -> Vec<PciDevice> {
    let devices = PCI_DEVICES.lock();
    devices
        .iter()
