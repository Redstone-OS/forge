//! # Synaptics I2C-HID Protocol
//!
//! Lógica específica para processar relatórios de touchpads Synaptics.
...
pub fn process_report(data: &[u8]) {
    ...
    // Formato típico Synaptics HID:
    // [report_id, buttons, contact_count, ...]
    // Cada contato: [confidence, tip, x_lo, x_hi, y_lo, y_hi, ...]
    ...
    if contact_count > 0 && data.len() >= 8 {
        ...
        let new_x = (x as i32 * state.screen_width) / 4096;
        let new_y = (y as i32 * state.screen_height) / 4096;
