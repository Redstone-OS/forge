...
// Helper para debug de tabelas (Manually walk the tables)
unsafe fn debug_print_mapping(vaddr: u64) {
    let cr3: u64;
    core::arch::asm!("mov {}, cr3", out(reg) cr3);
    let pml4_phys = cr3 & !0xFFF;
    ...
    crate::kdebug!("(DBG) Walking PT for vaddr=", vaddr);
    crate::kdebug!("(DBG) CR3 (Phys)=", pml4_phys);

    // PML4
    let pml4_ptr = pml4_phys as *const u64;
    let pml4e = *pml4_ptr.add(pml4_idx);
    crate::kdebug!("(DBG) PML4E[", pml4_idx as u64);
    crate::kdebug!("] = ", pml4e);

    if pml4e & 1 == 0 {
        crate::kdebug!("(DBG) PML4E Not Present");
        return;
    }
    ...
    let pdpt_ptr = pdpt_phys as *const u64;
    let pdpte = *pdpt_ptr.add(pdpt_idx);
    crate::kdebug!("(DBG) PDPTE[", pdpt_idx as u64);
    crate::kdebug!("] = ", pdpte);

    if pdpte & 1 == 0 {
        crate::kdebug!("(DBG) PDPTE Not Present");
        return;
    }
    ...
    let pd_ptr = pd_phys as *const u64;
    let pde = *pd_ptr.add(pd_idx);
    crate::kdebug!("(DBG) PDE[", pd_idx as u64);
    crate::kdebug!("] = ", pde);

    if pde & 1 == 0 {
        crate::kdebug!("(DBG) PDE Not Present");
        return;
    }
    ...
    let pte = *pt_ptr.add(pt_idx);
    crate::kdebug!("(DBG) PTE[", pt_idx as u64);
    crate::kdebug!("] = ", pte);

    if pte & 1 == 0 {
        crate::kdebug!("(DBG) PTE Not Present");
    } else {
        crate::kdebug!("(DBG) Final Phys Frame: ", pte & 0x000F_FFFF_FFFF_F000);
        let flags = pte & 0xFFF;
        crate::kdebug!("(DBG) Flags: ", flags);
    }
}
