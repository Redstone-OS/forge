...
pub unsafe fn load(data: &[u8]) -> Result<u64, Errno> {
    ...
    for i in 0..ph_count {
        ...
        if ph.typ == PT_LOAD {
            crate::kinfo!(
                "\x1b[35m[ELF]\x1b[0m PT_LOAD: vaddr={:#x} memsz={} filesz={}",
                ph.vaddr,
                ph.memsz,
                ph.filesz
            );
            ...
            // if ph.vaddr == 0 {
            //     crate::kwarn!("\x1b[35m[ELF]\x1b[0m Skipping segment with vaddr=0");
            //     continue;
            // }
            ...
            crate::kinfo!(
                "\x1b[35m[ELF]\x1b[0m Mapeando páginas {:#x} - {:#x}",
                start_page,
                end_page
            );
            ...
            while curr < end_page {
                ...
                if let Some((_phys, flags)) = vmm::translate_addr_with_flags(curr) {
                    ...
                    if flags & vmm::PAGE_USER != 0 {
                        crate::kinfo!(
                            "\x1b[35m[ELF]\x1b[0m Skipped alloc for {:#x} (already mapped USER, overlap)",
                            curr
                        );
                ...
                crate::kinfo!(
                    "\x1b[35m[ELF]\x1b[0m map_page({:#x}, {:#x}, {:#x})",
                    curr,
                    frame.addr,
                    page_flags
                );
                ...
                crate::kinfo!("\x1b[35m[ELF]\x1b[0m map_page OK, zerando...");
            ...
            crate::kinfo!("\x1b[35m[ELF]\x1b[0m Páginas mapeadas e zeradas");

            // Copiar dados do arquivo - loop manual para evitar copy_nonoverlapping
            if ph.filesz > 0 {
                crate::kinfo!(
                    "\x1b[35m[ELF]\x1b[0m Copiando {} bytes para {:#x}",
                    ph.filesz,
                    start_addr
                );
                ...
                crate::kinfo!("\x1b[35m[ELF]\x1b[0m Cópia OK");
            }
        }
    }
